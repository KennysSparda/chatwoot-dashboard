...
function statusDot(status: AgentAvailability) {
  return (
    <span
      className={clsx(
        "inline-block h-2 w-2 shrink-0 rounded-full 2xl:h-2.5 2xl:w-2.5",
        status === "online" &&
...
export default function AgentTable(props: AgentTableProps) {
  ...
  if (rows.length === 0) {
    return (
      <section className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] p-6 text-sm text-[var(--text-muted)] shadow-[var(--shadow-card)]">
      ...
      <div className="app-scrollbar min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
        <table className="w-full table-fixed border-collapse text-left">
          ...
          <thead className="sticky top-0 z-10 bg-[var(--card-bg)]">
            <tr className="border-b border-[var(--card-border)] text-[9px] uppercase tracking-[0.08em] text-[var(--text-muted)] 2xl:text-[10px]">
              <th className="px-3 py-2 font-medium 2xl:px-4 2xl:py-2.5">
                Agente
              </th>
              <th className="px-2 py-2 font-medium 2xl:py-2.5">Status</th>
              <th className="px-1 py-2 text-right font-medium 2xl:py-2.5">
                Abertas
              </th>
              <th className="px-1 py-2 text-right font-medium 2xl:py-2.5">
                Não at.
              ...
              <th className="px-2 py-2 text-right font-medium 2xl:px-3 2xl:py-2.5">
                1ª
              </th>
            </tr>
          </thead>
            ...
            {rows.map((row) => (
              ...
              >
                <td className="min-w-0 px-3 py-2.5 2xl:px-4 2xl:py-3">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <p
                        className="truncate text-xs font-semibold leading-tight text-[var(--text-main)] 2xl:text-[13px]"
                        title={row.name}
                      >
                        {row.name}
                      </p>
                      ...
                      <p
                        className="truncate text-[10px] leading-tight text-[var(--text-faint)] 2xl:text-[11px]"
                ...
                <td className="px-2 py-2.5 2xl:py-3">
                  <div
                    className={clsx(
                      "flex min-w-0 items-center gap-1.5 text-[11px] leading-tight 2xl:text-xs",
                ...
                <td className="px-1 py-2.5 text-right text-xs font-semibold tabular-nums text-[var(--text-main)] 2xl:text-sm">
